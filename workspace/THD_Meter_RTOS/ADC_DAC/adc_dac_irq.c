...
#include "private/adc_dac_header_priv.h"
	...
	void DMA_IRQHandler(void)
	{
			...
			if(Chip_GPDMA_Interrupt(LPC_GPDMA, dma_adc_int_canal) == SUCCESS) // Se fija si interrumpio el ADC externo
			{
				...
				{
					dma_adc_int_memory = dma_adc_int_memory_A;

					if(dma_adc_int_memory_B)								// Si hay para transferir por B, transfiero B y proceso A
					{
						dma_adc_int_status = STATUS_ADC_TRANS_B_PROC_A;

						dma_adc_int_descriptor = &dma_adc_int_descriptor_A;

						Chip_GPDMA_SGTransfer(LPC_GPDMA, dma_adc_int_canal,
												&dma_adc_int_descriptor_B,
												GPDMA_TRANSFERTYPE_P2M_CONTROLLER_DMA);
					}
					else													// Si NO hay para transferir por B, solo proceso A
					{
						dma_adc_int_status = STATUS_ADC_TRANS_B_PROC_A_ERR;
					}
				}
				else if(dma_adc_int_status == STATUS_ADC_TRANSFIRIENDO_B)	// Si SOLO estaba transfiriendo buffer B, ya termino de transferir.
				{
					dma_adc_int_memory = dma_adc_int_memory_B;
					dma_adc_int_descriptor = &dma_adc_int_descriptor_B;

					if(dma_adc_int_memory_B)								// Si hay para transferir por A, transfiero A y proceso B
					{
						dma_adc_int_status = STATUS_ADC_TRANS_A_PROC_B;

						Chip_GPDMA_SGTransfer(LPC_GPDMA, dma_adc_int_canal,
												&dma_adc_int_descriptor_A,
												GPDMA_TRANSFERTYPE_P2M_CONTROLLER_DMA);
					}
					else													// Si NO hay para transferir por A, solo proceso B
					{
						dma_adc_int_status = STATUS_ADC_TRANS_A_PROC_B_ERR;
					}
				}
				else
				{
					// Si termino de transferir antes de terminar procesamiento, doy una senal con PINGPONG_PROC2ERR para que
					// se continue con la transferencia luego de terminar procesamiento
					STATUS_ADC_PROC2ERR(dma_adc_int_status);

					#if DEBUG_MODE
						printf("[warning] ADC Interno interrumpio antes de terminar procesamiento\r\n");
					#endif
				}

				adc_int_pre_procesamiento();
			}
		#endif
